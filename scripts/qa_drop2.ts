// file: scripts/qa_drop2.ts
// description: Automated Drop 2 QA - drives the game in headless Chromium to verify gallery, onboarding, card earning, reveal, and persistence
// reference: src/ui/card_ui.ts, src/core/onboarding.ts, GAME_PLAN_V2.md
...
async function main(): Promise<void> {
  ...
  // 3. Sail at full ahead with no steering until she sinks (or timeout).
  await page.keyboard.press('KeyW');
  await page.keyboard.press('KeyW');
  ...
  for (let i = 0; i < 60; i++) {
    ...
    const over = await page.evaluate(() => {
      ...
      return Array.from(screens).some(
        (s) => !s.classList.contains('hidden') && s.textContent !== null && s.textContent.includes('SHE IS GONE'),
      );
    });
    if (over) {
      game_over = true;
  ...
  if (game_over) {
    ...
    const has_art = await page.evaluate(() => {
      const img = document.querySelector('.gameover-cards .card-art') as HTMLImageElement | null;
    ...
    // 4. Gallery shows earned cards.
    await page.keyboard.press('KeyG');
    await page.waitForTimeout(500);
    await page.locator('.gallery-tab').nth(1).click(); // Ships tab holds Maiden Voyage
    await page.waitForTimeout(300);
